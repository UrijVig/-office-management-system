...
import crm.example.models.entities.emplyee.dto.EmployeePasswordDTO;
import crm.example.services.EmployeeService;
...
public class EmployeeController {
     ...
     * 
     * @param dto   DTO с данными сотрудника
    ...
    @PreAuthorize("hasAnyRole('ADMIN', 'MANAGER')")
    @PostMapping
    public String saveEmployee(@Valid @ModelAttribute("employee") CreateEmployeeDTO dto,
            BindingResult bindingResult,
            Model model) {
        if (bindingResult.hasErrors()) {
            model.addAttribute("employee", dto);
            model.addAttribute("roles", employeeService.findAllRoles());    
            log.debug("ошибка заолнения формы при создании сотрудника {}", dto.getUsername());
            return "employees/employee_create_form";
        }
        try {
            log.info("Попытка сохранить пользователя {}", dto.getUsername());
        ...
        log.info("Пользователь {} сохранён успешно", dto.getUsername());
    ...
    public String getUpdateEmployeeForm(@PathVariable Long id, Model model) {
        ...
        model.addAttribute("employee", employeeService.getdtoById(id));
        model.addAttribute("roles", employeeService.findAllRoles());
     ...
     * 
     * @param dto   DTO с обновленными данными сотрудника
     * @param bindingResult объект для валидации данных формы
     * @param model         модель для передачи данных в представление
     * @return редирект на список сотрудников или возврат к форме при ошибках
     */
    @PreAuthorize("hasAnyRole('ADMIN', 'MANAGER')")
    @PostMapping("/update")
    public String updateEmployee(@Valid @ModelAttribute("employee") CreateEmployeeDTO dto,
            BindingResult bindingResult,
            Model model) {
        if (bindingResult.hasErrors()) {
            model.addAttribute("employee", dto);
            model.addAttribute("roles", employeeService.findAllRoles());
            log.debug("ошибка заолнения формы при редактировании сотрудника {}", dto.getId());
            return "employees/employee_update_form";
        }
        log.info("Попытка сохранить изменения пользователя {}", dto.getId());
        employeeService.updateEmployee(dto);
        log.info("Изменения пользователя {} сохранениы успешно", dto.getId());
        return "redirect:/employees";
    }
