...
На данном этапе необходимо подготовить проект к работе через REST API

- ~~Создание базовых rest для сигментов приложения~~
    - ~~Создание REST контроллеров для управления пользователми~~
    - ~~Создание REST контроллеров для управления оборудованием~~
    -~~ Создание REST контроллеров для управления рабочими местами~~
- Настройка безопасности
    - ~~Валидация данных~~
    - ~~Обработка ошибок~~
    - Авторизация через токены (отложено)

## 6 Этап - подготовка к развёртыванию приложения
- Рефокторинг кода
    - разделение логики рест сервера и клиента
        - ~~создать модуль рест сервера~~
        - ~~перенести логику управления базой данных и миграциями~~
        - ~~перенести логику безопасности~~
        - создать модуль приложения клиета для управления через формы/шаблоны
        - настроить общение между модулями
    - внедрение абстракции
        - внедрение интерфейсов для контроллеров и сервисов
        - общая структура для похожих классов
- Подключение внешней базы данных
- Настройка профилей работы приложения
- Настройка автотестирования
